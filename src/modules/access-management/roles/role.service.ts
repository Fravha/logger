...
export class RoleService {
  ...
  async create(data: CreateRoleDto, context: AuthenticatedAuditContext) {
    if (await this.repository.findByCode(data.code)) {
      throw new AppError("ROLE_CODE_ALREADY_EXISTS", "A role with this code already exists", 409);
    }
  ...
  async delete(id: string, context: AuthenticatedAuditContext) {
    ...
    if (role.code === "admin") {
      throw new AppError("SYSTEM_ROLE_PROTECTED", "The admin role cannot be deleted", 409);
    }

    const users = await this.repository.countUsers(id);
    if (users > 0) {
      throw new AppError("ROLE_IN_USE", "Role cannot be deleted while assigned to users", 409);
    }
